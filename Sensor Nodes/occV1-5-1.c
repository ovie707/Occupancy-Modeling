...
01/28/2018
v1-5-1a
changed timings for lingering measurements
grideye measurements every 5 seconds
stops measurements after 15 minutes of pir inactivity
background updates every hour
inactive if 30 mins if pir inactivity
active otherwise
*/
...
int sys_clock, m, i, j, k, sum, bg_timer = 0, read_pir_timer = 0, bg_pir_timer = 0;
volatile uint32_t ui32Loop;
#define SLAVE_ADDRESS 0x68
#define GRIDEYE_FREQ 1
#define BG_INTERVAL 15
#define READ_INACTIVITY 1
#define BG_INACTIVITY 5
...
void init_func(void)
{
	...
	TimerLoadSet(TIMER1_BASE, TIMER_A, sys_clock*GRIDEYE_FREQ -1);
...
void trigger(void)
{
	...
	if(bg_timer > BG_INTERVAL*60/GRIDEYE_FREQ)
	{
		if(bg_pir_timer > BG_INACTIVITY*60/GRIDEYE_FREQ) //if no activity
		{
			inactive_bg();
			bg_timer = 0;
		}
	...
	if(read_pir_timer < READ_INACTIVITY*60/GRIDEYE_FREQ)
	{
...
void pir(void)
{
	...
	{
		read_pir_timer = 0; //...reset the PIR inactivity timer
		bg_pir_timer = 0;
		pir_data = 1;
		//...toggle the blue led
	...
	else //If the PIR is inactive
	{
		read_pir_timer += 1; //...increment the inactivity timer
		bg_pir_timer += 1;
		pir_data = 0;
		if(read_pir_timer < READ_INACTIVITY*60/GRIDEYE_FREQ) //if readings are still being taken
		{
		...
		else //if readings aren't being taken (no activity)
		{
			//...toggle the red LED
			if(GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3))
				GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1|GPIO_PIN_2|GPIO_PIN_3, 0);
